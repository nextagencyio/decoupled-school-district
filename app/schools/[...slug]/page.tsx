...
import { GET_SCHOOL_BY_PATH } from '@/lib/queries'
...
interface SchoolByPathData {
  route: {
    entity: DrupalSchool
  } | null
}

async function getSchool(path: string): Promise<DrupalSchool | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<SchoolByPathData>({
      query: GET_SCHOOL_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/schools/${slug.join('/')}`
  const item = await getSchool(path)

  if (!item) {
    return { title: 'School Not Found | School District' }
  }

  return {
    title: `${item.title} | School District`,
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).principal && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).principal}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
