...
interface BoardMemberCardProps {
  item: DrupalBoardMember
}

export default function BoardMemberCard({ item }: BoardMemberCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).position && (
            <p className="text-xs text-primary-600 font-bold uppercase tracking-wide mb-2">{(item as any).position}</p>
          )}
